...
export default function CMSDashboardPage() {
  ...
  const [editingCOA, setEditingCOA] = useState<COAItem | null>(null);

  // Dynamic Technical Specifications Template
  const DEFAULT_TECH_SPECS = [
    { label: 'COMPOUND NAME', value: '' },
    { label: 'CAS NUMBER', value: '' },
    { label: 'MOLECULAR FORMULA', value: '' },
    { label: 'MOLECULAR WEIGHT', value: '' },
    { label: 'PURITY', value: '99.55%' },
    { label: 'FORM', value: 'Lyophilized powder' },
    { label: 'TESTING METHOD', value: 'HPLC-MS' },
  ];
  const [prodTechnicalSpecs, setProdTechnicalSpecs] = useState<{ label: string; value: string }[]>(DEFAULT_TECH_SPECS);
  ...
  const handleCreateProductSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    const cleanSpecs = prodTechnicalSpecs
    ...
    await addProduct({
      ...
      technicalSpecs: cleanSpecs.length > 0 ? cleanSpecs : DEFAULT_TECH_SPECS,
    ...
    setProdImage('');
    setProdDesc('');
    setProdTechnicalSpecs(DEFAULT_TECH_SPECS);
  };
    ...
    <div className="min-h-screen bg-white text-slate-900 flex font-sans select-none">
                ...
                <form onSubmit={handleCreateProductSubmit} className="bg-white p-6 sm:p-8 rounded-3xl border border-slate-300 space-y-4 shadow-xl text-slate-900">
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="sm:col-span-2">
                      <ImageUploadField
                        label="Product Compound Image"
                        value={prodImage}
                        onChange={setProdImage}
                        required
                        helperText="Upload image directly to your S3 bucket or paste an external URL link"
                      />
                    </div>
                    <div>
                    ...
                    <div className="sm:col-span-2">
                      ...
                      <textarea
                        rows={3}
                        placeholder="Detailed compound research notes, description..."
                        value={prodDesc}
                    ...
                    {/* Dynamic Technical Specifications / Technical Description Section */}
                    <div className="sm:col-span-2 bg-slate-50 p-4 sm:p-5 rounded-2xl border border-slate-200 space-y-3">
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-2 border-b border-slate-200 pb-2.5">
                        <div>
                          <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                            Technical Specifications (Header & Value Pairs)
                          </h4>
                          <p className="text-[11px] text-slate-500">
                            Add custom technical attributes displayed on the product details page
                          </p>
                        </div>
                        <div className="flex items-center space-x-2">
                          <button
                            type="button"
                            onClick={() => setProdTechnicalSpecs(DEFAULT_TECH_SPECS)}
                            className="text-[11px] font-bold px-3 py-1.5 rounded-lg bg-slate-200 hover:bg-slate-300 text-slate-800 cursor-pointer"
                          >
                            + Reset Template
                          </button>
                          <button
                            type="button"
                            onClick={() => setProdTechnicalSpecs([...prodTechnicalSpecs, { label: '', value: '' }])}
                            className="text-[11px] font-bold px-3 py-1.5 rounded-lg bg-slate-900 hover:bg-black text-white cursor-pointer flex items-center space-x-1"
                          >
                      ...
                      <div className="space-y-2.5 max-h-72 overflow-y-auto pr-1">
                        {prodTechnicalSpecs.map((spec, idx) => (
                          <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-xl border border-slate-200 shadow-xs">
                            <div className="w-2/5">
                              <input
                                type="text"
                                placeholder="Header (e.g. CAS NUMBER)"
                                value={spec.label}
                                onChange={(e) => {
                                  const updated = [...prodTechnicalSpecs];
                                  updated[idx].label = e.target.value;
                                  setProdTechnicalSpecs(updated);
                                }}
                                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 font-bold focus:outline-none focus:border-slate-900 uppercase"
                              />
                            </div>
                            <div className="w-3/5 flex items-center gap-1.5">
                              <input
                                type="text"
                                placeholder="Value (e.g. 49557-75-7)"
                                value={spec.value}
                                onChange={(e) => {
                                  const updated = [...prodTechnicalSpecs];
                                ...
                                className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1.5 text-xs text-slate-900 font-mono focus:outline-none focus:border-slate-900"
                              />
                              <button
                                type="button"
                                onClick={() => {
                                  setProdTechnicalSpecs(prodTechnicalSpecs.filter((_, i) => i !== idx));
                                }}
              ...
              {isAddingCOA && (
                ...
                >
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                    ...
                    <div className="sm:col-span-2">
                      <ImageUploadField
                        label="Report Certificate Image (HPLC / Mass Spec)"
                        value={coaImage}
                        onChange={setCoaImage}
                        required
                        helperText="Upload certificate image directly to S3 or paste an image link"
                      />
                    </div>
                  </div>
                  <div className="flex space-x-3 pt-2">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-6 text-xs">
                <div className="md:col-span-2">
                  <ImageUploadField
                    ...
                    onChange={(url) => setEditableSettings({ ...editableSettings, logoUrl: url })}
                    helperText="Upload transparent PNG or SVG logo to your S3 bucket or paste a URL"
                  />
                </div>
                ...
                <div className="md:col-span-2">
                  <ImageUploadField
                    label="Hero Background Image or Video (URL / S3 Upload)"
                    value={editableSettings.heroMediaUrl || ''}
                    onChange={(url) => setEditableSettings({ ...editableSettings, heroMediaUrl: url })}
                    helperText="Upload background image directly to your S3 bucket or paste an MP4 video / image link"
                  />
                </div>
      ...
      {viewingOrder && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-xs">
          <div className="bg-white border border-slate-300 rounded-3xl max-w-lg w-full text-slate-900 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden">
            <div className="flex justify-between items-center border-b border-slate-200 p-5 sm:px-6 shrink-0 bg-white">
              <div>
              ...
              <button onClick={() => setViewingOrder(null)} className="p-2 text-slate-400 hover:text-black hover:bg-slate-100 rounded-xl transition-colors cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            ...
            <div className="flex justify-between items-center border-t border-slate-200 p-4 px-6 bg-slate-50 shrink-0 text-sm font-black">
      ...
      {editingProduct && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-xs">
          <form
            ...
            className="bg-white border border-slate-300 rounded-3xl max-w-2xl w-full text-slate-900 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden"
          >
            <div className="flex justify-between items-center border-b border-slate-200 p-5 sm:px-6 shrink-0 bg-white">
              <div>
                <span className="text-[10px] text-[#FF007A] font-bold uppercase tracking-wider font-mono">PRODUCT CATALOG</span>
                <h3 className="text-base sm:text-lg font-black uppercase text-slate-900 truncate max-w-xs sm:max-w-md">Edit: {editingProduct.title}</h3>
              </div>
              <button type="button" onClick={() => setEditingProduct(null)} className="p-2 text-slate-400 hover:text-black hover:bg-slate-100 rounded-xl transition-colors cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-4 overscroll-contain">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                ...
                {/* Dynamic Technical Specifications in Edit Product */}
                <div className="sm:col-span-2 bg-slate-50 p-4 rounded-2xl border border-slate-200 space-y-3">
                  <div className="flex items-center justify-between border-b border-slate-200 pb-2">
                    <h4 className="text-xs font-black text-slate-900 uppercase tracking-wider">
                      Technical Specifications (Header & Value)
                    </h4>
                    <button
                      type="button"
                      onClick={() => {
                        const currentSpecs = editingProduct.technicalSpecs || [];
                        setEditingProduct({
                          ...editingProduct,
                          technicalSpecs: [...currentSpecs, { label: '', value: '' }],
                        });
                      }}
                      className="text-[11px] font-bold px-3 py-1 rounded-lg bg-slate-900 hover:bg-black text-white cursor-pointer flex items-center space-x-1"
                    >
                      <Plus className="w-3.5 h-3.5" />
                      <span>Add Field</span>
                    </button>
                  ...
                  <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
                    {(editingProduct.technicalSpecs && editingProduct.technicalSpecs.length > 0
                      ? editingProduct.technicalSpecs
                      : DEFAULT_TECH_SPECS
                    ).map((spec, idx) => (
                      <div key={idx} className="flex items-center gap-2 bg-white p-2 rounded-xl border border-slate-200 shadow-xs">
                        <div className="w-2/5">
                          <input
                            ...
                            value={spec.label}
                            onChange={(e) => {
                              const updated = [...(editingProduct.technicalSpecs || DEFAULT_TECH_SPECS)];
                              updated[idx] = { ...updated[idx], label: e.target.value };
                              setEditingProduct({ ...editingProduct, technicalSpecs: updated });
                            }}
                            className="w-full bg-slate-50 border border-slate-200 rounded-lg px-2.5 py-1 text-xs text-slate-900 font-bold uppercase"
                          />
                        </div>
                        <div className="w-3/5 flex items-center gap-1.5">
                          <input
                          ...
                          <button
                            type="button"
                            onClick={() => {
                              const updated = (editingProduct.technicalSpecs || DEFAULT_TECH_SPECS).filter((_, i) => i !== idx);
                              setEditingProduct({ ...editingProduct, technicalSpecs: updated });
                            }}
                            className="p-1 text-slate-400 hover:text-red-600 rounded-md cursor-pointer shrink-0"
                            title="Remove Field"
                          >
            ...
            <div className="flex justify-end items-center space-x-3 p-4 px-6 border-t border-slate-200 bg-slate-50 shrink-0">
              <button
                type="button"
                onClick={() => setEditingProduct(null)}
                className="px-5 py-2.5 bg-white hover:bg-slate-100 text-slate-700 text-xs rounded-xl font-bold border border-slate-300 transition-colors cursor-pointer"
              >
                Cancel
              </button>
              <button
                type="submit"
                className="bg-slate-900 hover:bg-black text-white text-xs px-6 py-2.5 rounded-xl font-bold cursor-pointer shadow-md hover:shadow-lg transition-all"
              >
                Save Changes
              </button>
            </div>
      ...
      {/* Edit COA Report Modal */}
      {editingCOA && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-3 sm:p-4 bg-black/70 backdrop-blur-xs">
          <form
            onSubmit={async (e) => {
              e.preventDefault();
              await updateCOA(editingCOA.id, editingCOA);
              setEditingCOA(null);
            }}
            className="bg-white border border-slate-300 rounded-3xl max-w-lg w-full text-slate-900 shadow-2xl flex flex-col max-h-[90vh] overflow-hidden"
          >
            <div className="flex justify-between items-center border-b border-slate-200 p-5 sm:px-6 shrink-0 bg-white">
              <h3 className="text-lg font-bold uppercase text-slate-900">Edit COA Report</h3>
              <button type="button" onClick={() => setEditingCOA(null)} className="p-2 text-slate-400 hover:text-black hover:bg-slate-100 rounded-xl transition-colors cursor-pointer">
                <X className="w-5 h-5" />
              </button>
            </div>
            <div className="p-5 sm:p-6 overflow-y-auto flex-1 space-y-4 overscroll-contain">
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-xs">
                <div className="sm:col-span-2">
                  ...
                  <input
                    type="text"
                    required
                    value={editingCOA.productTitle}
                    onChange={(e) => setEditingCOA({ ...editingCOA, productTitle: e.target.value })}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    required
                    value={editingCOA.batchNumber}
                    onChange={(e) => setEditingCOA({ ...editingCOA, batchNumber: e.target.value })}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900 font-mono"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="text"
                    required
                    value={editingCOA.purity}
                    onChange={(e) => setEditingCOA({ ...editingCOA, purity: e.target.value })}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900"
                  />
                </div>
                <div className="sm:col-span-2">
                  <ImageUploadField
                    label="Report Certificate Image (HPLC / Mass Spec)"
                    value={editingCOA.imageUrl}
                    onChange={(url) => setEditingCOA({ ...editingCOA, imageUrl: url })}
                    required
                    helperText="Upload certificate image directly to S3 or paste an image link"
                  />
                </div>
                <div className="sm:col-span-2">
                  <label className="block text-slate-700 font-bold mb-1">Report Date</label>
                  <input
                    type="text"
                    value={editingCOA.date}
                    onChange={(e) => setEditingCOA({ ...editingCOA, date: e.target.value })}
                    className="w-full bg-slate-50 border border-slate-300 rounded-xl px-3.5 py-2.5 text-slate-900"
                  />
                </div>
              </div>
            ...
            <div className="flex justify-end items-center space-x-3 p-4 px-6 border-t border-slate-200 bg-slate-50 shrink-0">
              <button type="button" onClick={() => setEditingCOA(null)} className="px-5 py-2.5 bg-white hover:bg-slate-100 text-slate-700 text-xs rounded-xl font-bold border border-slate-300 transition-colors cursor-pointer">
                Cancel
              </button>
              <button type="submit" className="bg-slate-900 hover:bg-black text-white text-xs px-6 py-2.5 rounded-xl font-bold cursor-pointer shadow-md hover:shadow-lg transition-all">
                Update COA Document
              </button>
            </div>
          </form>
